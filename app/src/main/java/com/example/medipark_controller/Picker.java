...
public class Picker extends LinearLayout {
    ...
    //        }
        ...
        //custom methods
        public void initPicker(float textSize, int textColor, int dividerColor ){

            //setTextSize for all TextView in Picker
            setAllTextSize(textSize);
            //setTextColor for all TextView in Picker
            setAllTextColor(textColor);
            setDividerColor(dividerColor);

        }
        public void setAllTextSize(float size){
            int count=this.getChildCount();
            for(int i=0;i<count;i++){
                View child=this.getChildAt(i);
                if(child instanceof EditText){
                    ((EditText) child).setTextSize(TypedValue.COMPLEX_UNIT_SP, size); // 글자 크기를 20sp로 설정
                }
            }
        }
        ...
        public void setAllTextColor(int color){
            ...
            for(int i=0;i<count;i++){
                ...
                if(child instanceof EditText){
                    try{
                        Field selectorWheelPaintField = this.getClass()
        ...
        public void setDisplayedDatas(String[] datas){
                this.setMinValue(0);
                this.setMaxValue(datas.length-1);
                this.setDisplayedValues(datas);
        }
